...
static PER *create_prioritized_replay(size_t capacity, double alpha, double beta) {
...
static void add_to_per(PER *p, PyObject *item, double priority, int use_priority) {
...
void calculate_sampling_priorities(
    const Batch *batch,
    double       tree_top_value,
    size_t       total_entry_count,
    double       beta) {
    if (total_entry_count == 0 || tree_top_value <= 0.0) {
        memset(batch->importance_weights, 0, batch->count * sizeof(batch->importance_weights[0]));
        return;
    }
...
Batch sample_from_per(PER *per, size_t batch_size) {
    ...
    calculate_sampling_priorities(&batch, tree_top_value, per->tree->num_entries, per->beta);
    return batch;
}
...
static PyObject *create_py_per(PyObject *self, PyObject *args, PyObject *kwargs) {
    (void)self;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "ndd", kwlist,
                                     &capacity_ss, &alpha, &beta)) {
        return NULL;
    }
...
static PyObject *py_per_sample(PyObject *self, PyObject *args) {
    ...
    for (size_t i = 0; i < batch.count; ++i) {
        ...
        PyObject *importance_weight = PyFloat_FromDouble(batch.importance_weights[i]);
...
static PyObject *py_per_update_priorities(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "OOO", &capsule, &td_errors_list, &indices_list)) {
        return NULL;
    }
    ...
    if (td_errors_count != indices_count) {
        PyErr_SetString(PyExc_ValueError, "td_errors and indices lists must have the same length");
        return NULL;
    }
    ...
    if (!td_errors.items) {
        PyErr_SetString(PyExc_MemoryError, "Failed to allocate memory for td_errors");
        return NULL;
    }
    ...
    if (!priority_indices) {
        ...
        PyErr_SetString(PyExc_MemoryError, "Failed to allocate memory for priority_indices");
        return NULL;
    }
...
static PyMethodDef PerMethods[] = {
    {"create", (PyCFunction)create_py_per, METH_VARARGS | METH_KEYWORDS, "Create a Prioritized Experience Replay (PER) instance."},
    {"size", py_per_size, METH_VARARGS, "Get the current size of the PER."},
    {"total_priority", py_per_total_priority, METH_VARARGS, "Get the total priority of the PER."},
    {"add", py_per_add, METH_VARARGS, "Add an item to the PER."},
    {"sample", py_per_sample, METH_VARARGS, "Sample a batch from the PER."},
    {"update_priorities", py_per_update_priorities, METH_VARARGS, "Update priorities in the PER."},
...
static struct PyModuleDef permodule = {
    ...
    "per",
    "Prioritized Experience Replay (C-based, generic Python items via PyCapsule)",
    -1,
    PerMethods
